...
def verify_publisher(files, certificate):
    public = subprocess.run(
        ["openssl", "x509", "-in", str(certificate), "-pubkey", "-noout"],
        check=True, capture_output=True,
    ).stdout
    compressed = subprocess.run(
        ["openssl", "ec", "-pubin", "-conv_form", "compressed", "-outform", "DER"],
        input=public, check=True, capture_output=True,
    ).stdout[-33:]
    ...
    for name in ("manifest.json", "app.elf"):
        if files[name][143:176] != compressed:
            raise ValueError("Archive signer does not match the configured publisher certificate")
...
def main():
    ...
    certificate = Path.home() / ".foundation" / "signing" / identity / f"{identity}.crt"
    verify_publisher(files, certificate)

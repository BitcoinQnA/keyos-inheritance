...
Passport Prime. It organises recovery instructions for a supported public
Bitcoin multisig wallet configuration. It is vendor-neutral and uses standard
BSMS or public descriptors for wallet interchange.
...
2. Compare the required signatures, network, fingerprints and first address
   against the same wallet in compatible multisig wallet software.
...
computer, finding signing keys, matching the wallet in compatible software, obtaining the
...
it never contacts wallet software or checks devices automatically.
...
This is an organiser, not a wallet or inheritance service. It cannot create or
activate a provider-managed inheritance plan, provide proof of life, contact an
...
for compatible wallet software, and `plan.json` for app restore. No plaintext sidecars or temporary
...
Import the decrypted `wallet.bsms` into compatible wallet software, not the
encrypted ZIP itself. Support for BSMS and complex timelocked descriptors varies
between wallet apps, so confirm compatibility before relying on the recovery plan.
...
- Two-path `wsh(or_d(multi(...),and_v(v:multi(...),after(...))))` inheritance
...
provider-managed or autonomous inheritance services, encrypted BSMS, proprietary wallet
...
python3 scripts/pack-beta3.py target/inheritance-sdk.app target/Inheritance-0.6.1-beta3.app
```
...
Wallet software is compatible when it can import the complete BSMS/descriptor
and reproduce the same network, policy and first receive address.
